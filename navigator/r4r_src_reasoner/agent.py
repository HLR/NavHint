...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, env, results_path, tok, episode_len=20, reasoner=None):
        ...
        self.reasoner = reasoner.cuda()
           
        # Optimizers
        self.vln_bert_optimizer = args.optimizer(self.vln_bert.parameters(), lr=args.lr)
        self.critic_optimizer = args.optimizer(self.critic.parameters(), lr=args.lr)
        self.trans_optimizer = AdamW(self.reasoner.parameters(), lr=args.lr)
        self.scheduler = get_linear_schedule_with_warmup(
           self.trans_optimizer, num_warmup_steps=1000, num_training_steps=args.iters
        )
        self.optimizers = (self.vln_bert_optimizer, self.critic_optimizer, self.trans_optimizer)

        # Evaluations
        self.losses = []
        self.criterion = nn.CrossEntropyLoss(ignore_index=args.ignoreid, size_average=False)
        self.ndtw_criterion = utils.ndtw_initialize()
        self.kl_losses = nn.KLDivLoss(reduction="batchmean")

        # Logs
        sys.stdout.flush()
        self.logs = defaultdict(list)
        self.reason_text = defaultdict(list)
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True):
        ...
        for t in range(self.episode_len):
            ...
            tmp_language_attention_mask = language_attention_mask
            visual_attention_mask = torch.cat((tmp_language_attention_mask, visual_temp_mask), dim=-1)
            ...
            visual_inputs = {'mode':              'visual',
                            ...
                            'lang_mask':          tmp_language_attention_mask,
                            'vis_mask':           visual_temp_mask,
                            'token_type_ids':     token_type_ids,
                            'action_feats':       input_a_t,
                            # 'pano_feats':         f_t,
                            'cand_feats':         candidate_feat}
            h_t, logit, vision_output = self.vln_bert(**visual_inputs)
            
            seq_tensor = torch.stack([ob['reason_encoding'] for ob in perm_obs], dim=0).cuda()
            seq_mask = torch.stack([ob['reason_mask'] for ob in perm_obs], dim=0).cuda()
            target_tensor = seq_tensor.clone().cuda()
            for ob_id, ob in enumerate(obs):
                try:
                    prompt_length = torch.where(seq_tensor[ob_id] == 50256)[0].item()
                    target_tensor[ob_id, :prompt_length] = 0
                except ValueError:
                    target_tensor[ob_id, -1] = 50256
            trans_loss, trans_hidden = self.reasoner.clipcap(vision_output, candidate_leng, seq_tensor, seq_mask, target_tensor)
            
            ### eval
            '''
            text_prompt = torch.stack([ob["prompt_encoding"] for ob in perm_obs], dim=0).cuda()
            prompt_mask = [ob["prompt_mask"] for ob in perm_obs]
            start_viewpint = [ob['viewpoint'] for ob in perm_obs]
            output_text = self.reasoner.infer_batch(vision_output, candidate_leng, text_prompt, prompt_mask)
            '''
            ### eval
            ...
            ### eval
            '''
            end_viewpint = [ob['viewpoint'] for ob in perm_obs]
            for ob_id, ob in enumerate(perm_obs):
                if start_viewpint[ob_id] == end_viewpint[ob_id]:
                    continue
                self.reason_text[ob['instr_id']].append((start_viewpint[ob_id], end_viewpint[ob_id], output_text[ob_id][0]))
            '''
            ### eval
    ...
    def train(self, n_iters, feedback='teacher', **kwargs):
        ...
        for iter in range(1, n_iters + 1):
            ...
            torch.nn.utils.clip_grad_norm(self.vln_bert.parameters(), 40.)
            torch.nn.utils.clip_grad_norm(self.reasoner.parameters(), 40.)
    ...
    def save(self, epoch, path):
        ...
        all_tuple = [("vln_bert", self.vln_bert, self.vln_bert_optimizer),
                    ...
                    ("reasoner", self.reasoner, self.trans_optimizer)]
