...
from reasoner import Translator
...
#PLACE365_FEATURES ='/egr/research-hlr/joslin/img_features/CLIP-ViT-B-32-views.tsv'
...
def train(train_env, tok, n_iters, log_every=2000, val_envs={}, aug_env=None):
    ...
    reasoner = Translator()
    #model_path = "/localscratch/zhan1624/CLIP_prefix_caption/checkpoints_reasoner_v_aug3/speaker/state_dict/90000_best_bleu.pt"
    #reasoner.load(model_path)
    listner = Seq2SeqAgent(train_env, "", tok, args.maxAction, reasoner=reasoner)
...
def valid(train_env, tok, val_envs={}):
    reasoner = Translator()
    agent = Seq2SeqAgent(train_env, "", tok, args.maxAction, reasoner=reasoner)
    # model_path = "/localscratch/zhan1624/CLIP_prefix_caption/checkpoints_reasoner_v_aug3/speaker/state_dict/90000_best_bleu.pt"
    # reasoner.load(model_path)
    ...
    for env_name, (env, evaluator) in val_envs.items():
        ...
        # json.dump(
        #     agent.reason_text,
        #     open(os.path.join("/localscratch/zhan1624/VLN-interactive/evaluation/", "eval_%s.json" % env_name), 'w'),
...
def train_val(test_only=False):
    ...
    train_env = R2RBatch(feat_dict, batch_size=args.batchSize, splits=['train'], tokenizer=tok, gpt_tokenizer=tok_gpt)
    ...
    val_envs = OrderedDict(
        ((split,
          (R2RBatch(feat_dict, batch_size=args.batchSize, splits=[split], tokenizer=tok, gpt_tokenizer=tok_gpt),
...
def train_val_augment(test_only=False):
    ...
    aug_env = R2RBatch(feat_dict, batch_size=args.batchSize, splits=[aug_path], tokenizer=tok_bert, name='aug', gpt_tokenizer=tok_gpt)
    train_env = R2RBatch(feat_dict, batch_size=args.batchSize, splits=['train'], tokenizer=tok_bert, gpt_tokenizer=tok_gpt)
    ...
    if test_only:
        ...
        val_env_names = ['val_seen', 'val_unseen']
    val_envs = {split: (R2RBatch(feat_dict, batch_size=args.batchSize, splits=[split], tokenizer=tok_bert, gpt_tokenizer=tok_gpt),
