...
from utils import length2mask, load_datasets, load_nav_graphs, pad_instr_tokens, pad_tokens
...
class R2RBatch():
    ...
    def __init__(self, feature_store, batch_size=100, seed=10, splits=['train'], tokenizer=None,
            ...
            for i_item, item in enumerate(tqdm(load_datasets([split]))):
                ...
                if "/" in split:
                    try:
                        ...
                        del new_item['reason_encoding']
                        del new_item['reason_mask']
                        # new_item['reason_encoding'] = item['reason_encoding']
                        # new_item['reason_mask'] = item['reason_mask']

                        # for key, value in item['reason_text'].items():
                        #     new_item['reason_encoding'][key.split("_")[0]] = torch.tensor(value)
                        # for key, value in item['reason_mask'].items():
                        #     new_item['reason_mask'][key.split("_")[0]] = torch.tensor(reason_mask)
                        ...
                        # new_item['reason_encoding'] = {}
                        # new_item['reason_mask'] = {}
                        
                        # for key, value in item['reason_text'].items():
                        #     reason_tokens = gpt_tokenizer.encode(instr+" "+value)
                        #     reason_tokens,reason_mask = pad_tokens(torch.tensor(reason_tokens), args.maxInput*2)
                        #     new_item['reason_encoding'][key.split("_")[0]] = torch.tensor(reason_tokens).tolist()
                        #     new_item['reason_mask'][key.split("_")[0]] = torch.tensor(reason_mask).tolist()
                    ...
                    for j, instr in enumerate(item['instructions']):
                            ...
                            gpt_instr_tokens = gpt_tokenizer.encode(instr)
                            #new_item['prompt_length'] = len(gpt_instr_tokens)
                            # gpt_instr_tokens, gpt_instr_mask = pad_tokens(torch.tensor(gpt_instr_tokens), args.maxInput+40)
                            # new_item['gpt_instr_encoding'] = gpt_instr_tokens
                            # new_item['gpt_instr_mask'] = gpt_instr_mask
                        
                            if split == "train" and item['reason_text']:
                                #gpt_instr_tokens = gpt_tokenizer.encode(instr)
                                
                                new_item['reason_encoding'] = {}
                                new_item['reason_mask'] = {}
                                new_item['prompt_length'] ={}
                                for key, value in item['reason_text'][j].items():
                                    #reason_tokens = gpt_tokenizer.encode(instr+value+" <|endoftext|> ")
                                    reason_tokens = gpt_tokenizer.encode(instr+" <|endoftext|> "+value)
                                    reason_tokens,reason_mask = pad_tokens(torch.tensor(reason_tokens), args.maxInput*2)
                                    new_item['reason_encoding'][key.split("_")[0]] = torch.tensor(reason_tokens)
                                    new_item['reason_mask'][key.split("_")[0]] = torch.tensor(reason_mask)
                                    
                            
                            else:
                                prompt_tokens = torch.tensor(gpt_tokenizer.encode(instr))
                                prompt_encoding, prompt_mask = pad_tokens(prompt_tokens, args.maxInput)
                                new_item['prompt_encoding'] = prompt_encoding
                                new_item['prompt_mask'] = len(prompt_tokens)
        ...
        # with open("/egr/research-hlr/joslin/r2r/data/reason_aug1.json", 'w') as f_out:
        #     json.dump(self.data, f_out)
    ...
    def _get_obs(self):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            if 'instr_encoding' in item:
                ...
                # obs[-1]['prompt_encoding'] = item['prompt_encoding']
                # obs[-1]['prompt_mask'] = item['prompt_mask']
                
                try:
                    obs[-1]['reason_encoding'] = item['reason_encoding'][state.location.viewpointId]
                    obs[-1]['reason_mask'] = item['reason_mask'][state.location.viewpointId]
                   
                except KeyError:
                    # obs[-1]['reason_encoding'] = torch.zeros(args.maxInput+40, dtype=torch.int64)
                    # obs[-1]['reason_mask'] = torch.zeros(args.maxInput+40, dtype=torch.int64)
                    # obs[-1]['prompt_length'] = 0
                    tmp_reason_enc = torch.zeros(args.maxInput*2, dtype=torch.int64)
                    tmp_reason_mask = torch.zeros(args.maxInput*2, dtype=torch.int64)
                    tmp_reason_enc[-1] = 50256
                    tmp_reason_mask[-1] = 1
                    obs[-1]['reason_encoding'] = tmp_reason_enc
                    obs[-1]['reason_mask'] = tmp_reason_mask
                

                    # obs[-1]['reason_encoding'] = item['gpt_instr_encoding']
                    # obs[-1]['reason_mask'] = item['gpt_instr_mask']
                    # obs[-1]['prompt_length'] = 0
                    # obs[-1]['prompt_encoding'] = item['prompt_encoding']
                    # obs[-1]['prompt_mask'] = item['prompt_mask']
