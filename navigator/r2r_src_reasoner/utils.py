...
import sys
import re
sys.path.append('/localscratch/zhan1624/VLN-interactive/build/')
import MatterSim
import string
import json
import time
...
def load_nav_graphs(scans):
    ...
    for scan in scans:
        if scan == ".vscode":
            continue
        with open('/localscratch/zhan1624/VLN-interactive/connectivity/%s_connectivity.json' % scan) as f:
            G = nx.Graph()
            positions = {}
            data = json.load(f)
...
def load_datasets(splits):
    ...
    for split in splits:
        ...
        if "/" not in split:
            #/localscratch/zhan1624/CLIP_prefix_caption/data_generation/reason_text_v2
            #/egr/research-hlr/joslin/r2r/reasoner/generation/
            # /egr/research-hlr/joslin/r2r/reasoner/generation_large/%s.json
            # /localscratch/zhan1624/CLIP_prefix_caption/data_generation/r2r/reason_text_no_sub/%s.jso
            with open('/localscratch/zhan1624/CLIP_prefix_caption/data_generation/r2r/reason_text_caption/%s.json' % split) as f:
...
def pad_instr_tokens(instr_tokens, maxlength=20, reason=True):
    ...
    if reason:
        instr_tokens = ['[SEP]'] + instr_tokens + ['[SEP]']
    else:
